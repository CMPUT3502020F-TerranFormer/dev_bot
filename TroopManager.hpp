...
public:
    TroopManager(threadsafe_priority_queue<Task>* t_queue, const ObservationInterface* obs)
        ...
        std::vector<Point2D> enemy_locations = observation->GetGameInfo().enemy_start_locations; // positions of possible enemy starting locations
    }
    ...
    void unitIdle(const Unit* unit) {
        ...
        case UNIT_TYPEID::TERRAN_BARRACKS: {
            ...
            if (CountUnitType(UNIT_TYPEID::TERRAN_MARINE) < 25) {
                task_queue->push(Task(TRAIN, ATTACK_AGENT, 5, ABILITY_ID::TRAIN_MARINE, UNIT_TYPEID::TERRAN_MARINE, UNIT_TYPEID::TERRAN_BARRACKS, unit->tag));
            }

            task_queue->push(Task(TRAIN, ATTACK_AGENT, 5, ABILITY_ID::TRAIN_MARAUDER, UNIT_TYPEID::TERRAN_MARAUDER, UNIT_TYPEID::TERRAN_BARRACKS, unit->tag));

            break;
        }

        case UNIT_TYPEID::TERRAN_STARPORT: {
            task_queue->push(Task(TRAIN, ATTACK_AGENT, 5, ABILITY_ID::TRAIN_BANSHEE, UNIT_TYPEID::TERRAN_BANSHEE, UNIT_TYPEID::TERRAN_STARPORT, unit->tag));
            task_queue->push(Task(TRAIN, ATTACK_AGENT, 5, ABILITY_ID::TRAIN_MEDIVAC, UNIT_TYPEID::TERRAN_MEDIVAC, UNIT_TYPEID::TERRAN_STARPORT, unit->tag));
        }
        }
    }
